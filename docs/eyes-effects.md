...
Inference completion now coalesces a request on the preview engine thread. During playback the next naturally decoded frame consumes the result; completion no longer sends a pause command. A paused viewer resubmits its latest requested cursor only if no newer transport request is pending. The worker releases its busy gate before notifying the viewer, eliminating the delayed duplicate refresh and cooldown window that could discard a seek-time sample. Preset changes no longer recreate resident shaders, decoders and tracking models; graph topology/qualifier changes retain their rebuild behavior. Device testing of first-frame latency remains required.
