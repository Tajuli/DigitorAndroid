 ...
 *
 * Ordinary Serial nodes are spliced out of their current path (previous -> next).
 * Any editable branch that feeds a two-branch Mix collapses the remaining branch back to Serial
 * and removes the now-unnecessary Mix node. Multi-branch mixes stay intact while 2+ branches remain.
 */
fun ClipNodeGraph.deleteEditableNodeV4(nodeId: String): ClipNodeGraph {
    ...
    // The first branch created by addParallelNode intentionally retains SERIAL kind while the added
    // branch is PARALLEL. Detect the topology (branch -> Mix), not only the enum, so deleting either
    // side of a two-branch parallel group produces the same clean collapse.
    val mix = outgoing
