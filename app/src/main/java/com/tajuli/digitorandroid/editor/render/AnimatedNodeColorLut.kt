 ...
 *
 * Preview keeps the same LUT effect/GL texture attached to ExoPlayer. Static color edits are
 * detected from the live node-graph hash and upload into the existing texture on the next rendered
 * frame. Animated grades continue to evaluate by source timestamp. Export always uses the immutable
 * clip passed to Transformer.
 */
@UnstableApi
