...
/**
 * V36 persistent portrait renderer.
 *
 * The two preview stages are intentionally attached before any beauty filter is selected. A filter
 * tap therefore changes only live effect-marker strengths; it does not wait for a new MediaCodec/GL
 * session and it does not wait for ML analysis. BASE has an immediate YCbCr skin-probability path,
 * then automatically refines to temporal semantic skin masks as MediaPipe results arrive. FINISH
 * similarly starts as pass-through and begins using face/hair geometry as analysis is cached.
 *
 * Compared with V34, Skin Bright has substantially more midtone headroom while keeping a strong
 * highlight shoulder and gamut-safe hue vector. At 100% it is calibrated to reach the visibly
 * brighter face level in the supplied CapCut reference without bleaching an already-white wall.
 */
@UnstableApi
internal class BeautyFaceEffectV36 private constructor(
    ...
    companion object {
        fun baseForClip(clip: TimelineClip, preview: Boolean): BeautyFaceEffectV36? {
            val s = clip.beautyStrengthsV28()
            val active = s.skinBright > .001f || s.skinSmooth > .001f
            return if (preview || active) BeautyFaceEffectV36(clip, preview, Stage.BASE) else null
        }

        fun finishForClip(clip: TimelineClip, preview: Boolean): BeautyFaceEffectV36? {
            val s = clip.beautyStrengthsV28()
            val active = s.pinkLip > .001f || s.hairBrowDark > .001f || s.eyePop > .001f
            return if (preview || active) BeautyFaceEffectV36(clip, preview, Stage.FINISH) else null
        }
    }
    ...
    private class Program(
        ...
        override fun drawFrame(inputTexId: Int, presentationTimeUs: Long) {
            try {
                ...
                val skinBracket = if (needsSkin) skinBracket(currentClip, sourceUs) else SkinBracket.empty()
                ...
                val skinReferenceA = skinBracket.a?.let { refreshFaceTrack(currentClip)?.geometryAt(it.sourceTimeUs) }
                val skinReferenceB = skinBracket.b?.let { refreshFaceTrack(currentClip)?.geometryAt(it.sourceTimeUs) }
                ...
                val hairBracket = if (needsHair) hairBracket(currentClip, sourceUs) else HairBracket.empty()
                ...
                val hairReferenceA = hairBracket.a?.let { refreshFaceTrack(currentClip)?.geometryAt(it.sourceTimeUs) }
                val hairReferenceB = hairBracket.b?.let { refreshFaceTrack(currentClip)?.geometryAt(it.sourceTimeUs) }
                val hairWarpA = maskWarp(geometry, hairReferenceA)
                val hairWarpB = maskWarp(geometry, hairReferenceB)

                val stageActive = if (baseStage) {
                    strengths.skinBright > .001f || strengths.skinSmooth > .001f
                } else {
                    strengths.pinkLip > .001f || strengths.hairBrowDark > .001f || strengths.eyePop > .001f
                }
                ...
                program.setFloatUniform("uStageActive", if (stageActive) 1f else 0f)
        ...
        private fun skinBracket(currentClip: TimelineClip, sourceUs: Long): SkinBracket {
            val frames = BeautyFaceSkinMaskStoreV31.index(appContext, currentClip).frames
            if (frames.isEmpty()) return SkinBracket.empty()
            if (frames.size == 1) return SkinBracket(frames[0], frames[0], 0f)
            var rightIndex = frames.binarySearchBy(sourceUs) { it.sourceTimeUs }
            if (rightIndex >= 0) return SkinBracket(frames[rightIndex], frames[rightIndex], 0f)
            rightIndex = -rightIndex - 1
            val right = frames.getOrNull(rightIndex)
            val left = frames.getOrNull(rightIndex - 1)
            if (left == null) return SkinBracket(right, right, 0f)
            if (right == null) return SkinBracket(left, left, 0f)
            ...
            return SkinBracket(left, right, mix)
        }

        private fun hairBracket(currentClip: TimelineClip, sourceUs: Long): HairBracket {
            val frames = BeautyHairMaskStoreV29.index(appContext, currentClip).frames
            if (frames.isEmpty()) return HairBracket.empty()
            if (frames.size == 1) return HairBracket(frames[0], frames[0], 0f)
            var rightIndex = frames.binarySearchBy(sourceUs) { it.sourceTimeUs }
            if (rightIndex >= 0) return HairBracket(frames[rightIndex], frames[rightIndex], 0f)
            rightIndex = -rightIndex - 1
            val right = frames.getOrNull(rightIndex)
            val left = frames.getOrNull(rightIndex - 1)
            if (left == null) return HairBracket(right, right, 0f)
            if (right == null) return HairBracket(left, left, 0f)
            val span = (right.sourceTimeUs - left.sourceTimeUs).coerceAtLeast(1L)
            val mix = ((sourceUs - left.sourceTimeUs).toDouble() / span.toDouble()).toFloat().coerceIn(0f, 1f)
            return HairBracket(left, right, mix)
        }
        ...
        private fun maskWarp(current: BeautyFaceGeometryV28?, reference: BeautyFaceGeometryV28?): MaskWarp {
            if (current == null || reference == null) return MaskWarp.identity()
        ...
        private fun faceScaleWarp(current: BeautyRectV28, reference: BeautyRectV28): MaskWarp {
        ...
        private data class SkinBracket(
            val a: BeautyFaceSkinMaskFrameV31?,
            val b: BeautyFaceSkinMaskFrameV31?,
            val mix: Float,
        ) { companion object { fun empty() = SkinBracket(null, null, 0f) } }

        private data class HairBracket(
            val a: BeautyHairMaskFrameV29?,
            val b: BeautyHairMaskFrameV29?,
            val mix: Float,
        ) { companion object { fun empty() = HairBracket(null, null, 0f) } }

        private data class MaskWarp(
            val matrix: FloatArray,
            val translation: FloatArray,
        ) {
            companion object {
                fun identity() = MaskWarp(
        ...
        companion object {
            private const val FACE_TRACK_REFRESH_MS = 120L
            ...
            private const val FRAGMENT_SHADER = """
                ...
                float skinProbability(vec3 c) {
                    ...
                    float cbBand = smoothstep(.235, .300, cb) * (1.0 - smoothstep(.525, .595, cb));
                    float crBand = smoothstep(.335, .400, cr) * (1.0 - smoothstep(.675, .740, cr));
                    return cbBand * crBand * smoothstep(.045, .16, y) * (1.0 - smoothstep(.95, 1.0, y));
                }
                ...
                vec3 relightPreserveHue(vec3 c, float targetY, float amount) {
                    ...
                    float highlightDesat = smoothstep(.60, .95, targetY) * .16 * amount;
                ...
                void main() {
                    ...
                    float faceWide = uHasFace > .5 ? ellipseMask(p, uFaceRect, .98, 1.48) : 1.0;
                    ...
                    // Important V36 behavior: when semantic analysis has not finished yet, color-based
                    // skin selection works immediately. If face geometry is already available it is
                    // used as a spatial prior; otherwise likely skin anywhere in frame can respond.
                    float fallbackSkin = skinProbability(rgb) * faceWide;
                    ...
                    float skinAlpha = skin * (.42 + .58 * skin);
                    ...
                    if (uSkinSmooth > .001) {
                        ...
                        vec3 textureSafe = base + detail * mix(.78, .58, clamp(amount, 0.0, 1.0));
                        float edgeSignal = abs(luma(rgb) - luma(fine)) * 4.0;
                        float edgeProtect = 1.0 - smoothstep(.08, .30, edgeSignal);
                        float mixAmount = clamp(skinAlpha * edgeProtect * amount * .34, 0.0, .40);
                    ...
                    if (uSkinBright > .001) {
                        ...
                        float y = luma(rgb);
                        float blackGate = smoothstep(.045, .17, y);
                        float highlightProtect = 1.0 - smoothstep(.72, .965, y);
                        float headroom = pow(max(1.0 - y, 0.0), .68);
                        float lift = .190 * amount * headroom * blackGate * (.62 + .38 * highlightProtect);
                        float targetY = min(y + lift, .965);
                        vec3 lifted = relightPreserveHue(rgb, targetY, amount);
                        float mixAmount = clamp(skinAlpha * (.82 + .12 * min(amount, 1.0)), 0.0, .96);
