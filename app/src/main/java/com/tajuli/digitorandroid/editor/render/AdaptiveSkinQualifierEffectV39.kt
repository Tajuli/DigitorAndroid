        ...
        companion object {
            ...
            private const val FRAGMENT_SHADER = """
                ...
                float qualifierWeight(vec3 c, vec2 center) {
                    ...
                    float satGate = smoothstep(.020, .085, span);
                    float darkGate = smoothstep(.075, .24, y);
                    float whiteGate = 1.0 - smoothstep(.91, .992, y);
                ...
                vec3 relightAndPale(vec3 c, float targetY, float q) {
                    float y = luma(c);
