...
source = np.full((h,w,4), 35, np.uint8); source[:,:,3] = 179
...
def amounts(index, strength=1):
    ...
    for name,part in zip(['uEyesA','uEyesB','uEyesC'],[a[:4],a[4:8],a[8:]]): vec(name,part)
...
for index in range(12):
    ...
    assert np.max(out[:,:,:3]-source[:,:,:3])>40, ('Invisible',index)
    assert np.array_equal(out[:,:,3],source[:,:,3]),('Alpha',index)
    vec('uEyeState',[0,0,0,0]); assert np.array_equal(render(),source),('Blink',index)
...
print('PASS: production shader compiles/links; 12 distinct visible effects; zero, blink, missing face, alpha and tracking checks pass.')
